...
namespace UnityTechRaw.KartAndFPS.Assets.FPS.Scripts
{
    ...
    public class PlayerWeaponsManager : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (activeWeapon)
            {
                if (Physics.Raycast(weaponCamera.transform.position, weaponCamera.transform.forward, out var hit, 1000, -1, QueryTriggerInteraction.Ignore))
        ...
        private void UpdateWeaponBob()
        {
            ...
            {
                var playerCharacterVelocity = (m_PlayerCharacterController.transform.position - m_LastCharacterPosition) / Time.deltaTime;

                // calculate a smoothed weapon bob amount based on how close to our max grounded movement velocity we are
                var characterMovementFactor = 0f;
                if (m_PlayerCharacterController.isGrounded)
                    characterMovementFactor = Mathf.Clamp01(
                        playerCharacterVelocity.magnitude / (m_PlayerCharacterController.maxSpeedOnGround * m_PlayerCharacterController.sprintSpeedModifier)
                    );
                ...
                var vBobValue = (Mathf.Sin(Time.time * frequency * 2f) * 0.5f + 0.5f) * bobAmount * m_WeaponBobFactor;
        ...
        public bool AddWeapon(WeaponController weaponPrefab)
        {
            ...
            for (var i = 0; i < m_WeaponSlots.Length; i++)
                ...
                if (m_WeaponSlots[i] == null)
                {
                    ...
                    var layerIndex = Mathf.RoundToInt(Mathf.Log(FPSWeaponLayer.value, 2)); // This function converts a layermask to a layer index
                    foreach (var t in weaponInstance.gameObject.GetComponentsInChildren<Transform>(true)) t.gameObject.layer = layerIndex;
