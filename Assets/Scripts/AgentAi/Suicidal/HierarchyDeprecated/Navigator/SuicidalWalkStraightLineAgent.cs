...
namespace AgentAi.Suicidal.HierarchyDeprecated.Navigator
{
    ...
    {
        [CanBeNull] private IDynamicObjectOfInterest _currentTarget;
        ...
        {
            if (!_previousClosestDistance.HasValue)
            {
                _previousClosestDistance = GetCurrentDistanceFromTarget();
                return;
            }
            ...
            if (distance.HasValue && distance < _previousClosestDistance)
            {
                var distanceDifference = _previousClosestDistance.Value - distance.Value;
        ...
        private void CheckReachedTarget()
        {
            ...
            if (_unit.Bounds.Intersects(_currentTarget.InterestedInformation.Bounds))
            {
#if TRAINING
                Done();
#else
